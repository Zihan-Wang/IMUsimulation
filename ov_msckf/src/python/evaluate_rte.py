...
'''
Example usage: 
python evaluate_rte.py ~/datasets/Kitti/dataset/poses/06_converted.txt \
~/workspace/catkin_imu/src/IMUsimulation/ov_data/sim/imupose.csv --plot kitti_odometry_06_rte --fixed_delta
'''
if __name__ == '__main__':
    ...
    if args.verbose:
        ...
        print "translational_error.mean %f m"%numpy.mean(trans_error)
        print "translation_error.rmse %f m"%numpy.sqrt(numpy.dot(trans_error,trans_error) / len(trans_error))
    ...
    if args.plot:    
        ...
        ax1.plot(stamps - stamps[0],trans_error,'-',color="blue")
        ax1.set_xlabel('time [s]')
        ax1.set_ylabel('translational error [m]')

        # rotation error
        ax2 = fig.add_subplot(212)
        ax2.plot(stamps - stamps[0], rot_error * 180.0 / numpy.pi, '-', color="red")
        ax2.set_xlabel('time [s]')
        ax2.set_ylabel('rotational error [deg]')
